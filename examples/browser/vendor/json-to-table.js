 ...
 * REF: https://github.com/afshinm/Json-to-HTML-Table
 */
...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText) {
  ...
  var tbl = '<table cellspacing="0"' + idMarkup + classMarkup + '>{0}{1}</table>';
    ...
    if (isStringArray)
      ...
      if (typeof(parsedJson[0]) == 'object') {
        ...
        for (i = 0; i < headers.length; i++) {
          thCon += thRow.format(capitalize(headers[i]));
        }
      }
    }
      ...
      if (headers) {
        ...
        if (parsedJson.length == 1) {
          for (j = 0; j < headers.length; j++) {
            ...
            tbCon += tdRow.format(capitalize(headers[j]));
            if (value) {
              if (typeof(value) == 'object') {
                tbCon += tdRow.format(JSON.stringify(value)
                  .replace(/\{|\}|"/ig, '')
                  .replace(/\:/g, ': ')
                  .replace(/,(?! )/g, '<br/>')
